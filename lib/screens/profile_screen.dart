...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  bool _isUpdatingPhoto = false; // Track photo update state
    ...
    try {
      ...
      if (base64Image != null) {
        ...
        if (result == null) {
          // Success
          setState(() {
            _currentUserPhotoURL = base64Image;
          });
          if (mounted) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Failed to update profile picture: $result'),
  ...
  void confirmOTP(context, otpCode) async {
    await fbService.confirmUpdateOTP(smsCode: otpCode).catchError((e) {
      ...
      return null; // Return null for the error case
    });
  }
                  ...
                  TextButton(
                    onPressed: () async {
                      if (dialogFormKey.currentState!.validate()) {
                        ...
                        // Update name in Firestore and Firebase Auth if changed
                        final userData =
                        ...
                        // Trim the input name to remove extra whitespace
                        final trimmedName = name.trim();

                        if (trimmedName.isNotEmpty &&
                            trimmedName != currentName) {
                          try {
                            // Use the new method that updates both Firebase Auth and Firestore
                            final result = await fbService
                                .updateUserDisplayName(trimmedName);
                            if (result != null) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text('Error updating name: $result'),
                                ),
                              );
                              return;
                            }
                  ...
                  children: [
                        ...
                        children: [
                          FutureBuilder<Map<String, dynamic>?>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              if (photoURL != null &&
                                  ImageService.isValidBase64DataUrl(photoURL)) {
                                // Display base64 image
                                return ImageService.buildCircularAvatar(
                                  base64DataUrl: photoURL,
                                  radius: 60,
                                );
                              } else {
                                // Default image
                    ...
                    Center(
                      child: GestureDetector(
                        onTap: _isUpdatingPhoto ? null : _updateProfilePicture,
                        child: Text(
                          _isUpdatingPhoto ? 'Updating...' : 'Edit picture',
                          style: TextStyle(
                            fontFamily: 'CenturyGo',
                            fontSize: 14,
                            color:
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              TextFormField(
                                ...
                                decoration: InputDecoration(
                                  hintText:
                                      'Enter your name (letters, spaces, hyphens, apostrophes only)',
                                  filled: true,
